...
const MATCH_NAME = /<[a-zA-Z\s\\']+>/;
const MATCH_PHONE = /\+\d{1}[\d-]+/;
const MATCH_EXCLUDED_CHARS = /[^a-zA-Z\d\s-_.]/g;

function find(entries, num) {
  return entries.filter(e => e.indexOf(num) > -1);
}

function getName(entry) {
  const [name] = entry.match(MATCH_NAME);
...
function getNumber(entry) {
  return entry.match(MATCH_PHONE)[0].replace('+', '');
}
...
function phone(strng, num) {
  const results = find(strng.split('\n'), num);
  ...
  return `Phone => ${getNumber(entry)}, Name => ${getName(entry)}, Address => ${getAddr(entry)}`;
}
